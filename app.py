...
def login():
    if request.method == 'POST':
        ...
        if user_type == "admin":
            ...
            if admin and verify_password(admin.password_hash, p):
                login_user(admin)
                log_action(admin.username, 'login')
                return redirect(url_for('index'))

        # -------- Employee Login --------
        if user_type == "employee":
            emp = Employee.query.filter(
                (Employee.email == u) | (Employee.contact == u)
            ).first()

            if emp and emp.password_hash and verify_password(emp.password_hash, p):
                login_user(emp)
...
def add_employee():
    ...
    if emp:
        ...
        emp = Employee(emp_code=code, first_name=f.get('first_name'), last_name=f.get('last_name'),
                       ...
                       password_hash=hash_password(f.get('contact')),
                       photo = filename)
...
def employee_leave_request():
    ...
    start = datetime.date.fromisoformat(request.form.get('start_date'))
    end = datetime.date.fromisoformat(request.form.get('end_date'))
...
@app.route('/employee/payroll/<int:pid>/pdf')
@login_required
def employee_payroll_pdf(pid):
    if isinstance(current_user, Admin):
        return redirect(url_for('index'))

    payroll = Payroll.query.get_or_404(pid)

    if payroll.employee_id != current_user.id:
        return "Unauthorized", 403

    return payroll_pdf(pid)


# -------- Your original status route below --------

@app.route('/status')
def status():
    return jsonify({'ok':True,'version':'hr-1.0'})
